...
void dezalocareVectorFarmacie(struct Farmacie** vector, int* nrFarmacii) {
	for (int i = 0; i < *nrFarmacii; i++) {
		free((*vector)[i].nume);
	}
...
//	int v2* = ;
	...
	for (int i = 0; i < nrFarmacii; i++) {
		farmacii[i] = initializareFarmacie(i + 1, "Farmacii", 30*i+10);
	}
	...
	dezalocareVectorFarmacie(&farmaciiCopiate, &farmaciiCopiate);
